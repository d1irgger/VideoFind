...
class GlassVideoFinder(ctk.CTk):
    ...
    def build_page2(self):
        ...
        card = ctk.CTkFrame(container, fg_color=(Config.CARD_LIGHT, Config.CARD_DARK), corner_radius=12)
        card.pack(fill="x", padx=20, pady=10)

        sw_frame = ctk.CTkFrame(card, fg_color="transparent")
        sw_frame.pack(fill="x", padx=16, pady=12)

        ctk.CTkLabel(sw_frame, text="扫描子文件夹", font=ctk.CTkFont(size=15),
                     text_color=(Config.TEXT_PRIMARY_LIGHT, Config.TEXT_PRIMARY_DARK)).pack(side="left")

        self.subfolder_var = ctk.BooleanVar(value=self.scanner.scan_subfolder)
        sw = ctk.CTkSwitch(sw_frame, text="", variable=self.subfolder_var,
        ...
        ctk.CTkLabel(container, text="开启：遍历所有子文件夹\n关闭：仅扫描所选文件夹根目录",
                     ...
                     text_color=(Config.TEXT_SECONDARY_LIGHT, Config.TEXT_SECONDARY_DARK),
                     justify="left").pack(anchor="w", padx=20, pady=(5, 20))
    ...
    def load_animate(self, step):
        ...
        self.after(150, self.load_animate, step + 1)  # 从300ms缩短为150ms
